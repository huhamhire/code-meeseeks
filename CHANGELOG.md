...
- **评论 emoji 反应**：评论下方展示已有 emoji 反应（含计数与本人标记），可点选添加 / 取消；GitLab / Bitbucket 支持搜索挑选更多表情，GitHub 为其固定表情集——随平台能力提供，不支持的平台自动隐藏。
- **评论 @提及自动补全**：撰写评论 / 回复时输入 `@` 可从本 PR 参与者（评论与提交作者）中选择补全；仍可自由手打任意用户名。
- **评论图片附件**：撰写评论 / 回复时可粘贴图片或点输入框右上角的图片按钮选取上传，自动插入正文（随平台能力提供，不支持的平台隐藏入口）。
- **评论表情代码渲染**：评论正文里的 `:tada:` 等表情代码按内置表情集渲染为对应 emoji（代码块与未知代码原样保留）。
