...
- **按提交发起评审 Agent**：Diff 视图的变更范围选择器中，各提交行新增「评审 / 改进 / 提问」动作，可将 review / improve / ask 的 diff 范围限定在该提交自身改动（`parent..sha`）而非 PR 全量；「提问」会把提交范围挂到输入栏、输入问题后针对该提交作答。评审结果卡展示所限定提交的范围徽标。
