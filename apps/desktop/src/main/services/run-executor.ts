...
import type { QueueItem } from './run-queue-types.js';
import {
...
/**
 * pr-agent run 的**执行器**（与队列调度 RunQueueService 分离）：给定一个已 dequeue 的队列项，落盘
 * startReviewRun → 物化 worktree → 组装 env/提示词 → bridge.run → 读产物解析 → finishReviewRun 收尾。
