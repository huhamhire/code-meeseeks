...
- `connection`: `capabilities()` (static capability descriptor, see §2), `ping()` (version + user), `getCurrentUser()` (synchronously reads the ping cache, used for the approved check), `getCloneUrl()`, `searchUsers(query, repo)` (user search for `@mention` autocomplete — repo-scoped where callable by a normal reviewer, else instance-wide; gated by the `userSearch` capability).
...
| fileLevelComments | yes (anchor without a line) | yes (`subject_type: "file"`) | no (no file-level diff-comment API) |
