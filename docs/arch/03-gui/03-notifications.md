...
把「与我相关」的事件从应用内提示扩展到操作系统层：被 @、被回复、有新 PR 时弹原生系统通知（toast），并在 macOS dock 图标上显示「待回应」计数角标。用户向使用说明见 [docs/guide/](../../guide/README.md)；列表内的未读点名计数见 [状态存储](../99-core/01-state-storage.md) 的「未读点名计数」。
