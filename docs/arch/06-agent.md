...
不负责：pr-agent 进程本身与 token 采集（见 [04](04-pragent-runtime.md)）、findings
解析与草稿发布（见 [05](05-review-workflow.md)）、规则匹配的正则语义（见 [07](07-rules.md)；
...
- **读写边界清晰**：`SOUL` 仅人可改（Agent 只读）；`AGENTS` / `rules/` 人写为主；`MEMORY` / `USER`
  是 Agent 与人共写的可写记忆。
- **整目录团队共享**：与 [07](07-rules.md) 同理——把 `agent.dir` 指向一个 git repo，团队 clone
...
**每次 Agent 执行都现读、现装配，无缓存**（与 [07](07-rules.md)「每次 run 现读规则」一致），
...
8. **语言行为指令** —— 执行时注入的显式国际化规则，覆盖两类语言行为：
   - **AI 输出语言**：Agent / 评审产物用目标语言输出，跟随 `config.language` /
     `resolveLanguage`（沿用既有「AI 回复语言随界面语言」，见 [04](04-pragent-runtime.md)
     的响应语言注入、[11](11-i18n.md)）。
...
Profile 凭据、出站代理与 token 采集；见 [04](04-pragent-runtime.md)、[09](09-networking-proxy.md)）
。一次会话：
...
- **运行态可见且共享**：PR 会话**直接展示该 PR 下正在执行的自动任务**——AutoPilot / Agent 编排出的
  ...
  run-state store（见 [05](05-review-workflow.md)、[10](10-ui-interaction.md)）与同一条运行队列 /
...
- **会话隔离**：每个 PR 的 Agent 会话状态（todo、进度、plan、transcript）**按 PR 隔离**，落在该 PR
  的 per-PR 目录下（见 [03](03-state-storage.md) 的 `state/prs/<hash>/`），互不串扰。不同 PR 并发跑
  Agent 安全。
...
- **pr_agent 工具运行层 —— 并行**：`/describe`·`/review`·`/ask` 子进程仍走既有**共享运行队列**（见
  [05](05-review-workflow.md) 的 `max_concurrency` + worktree 并发模型），**可并行调用**——跨 PR、
...
**配置（`agent.*` 命名空间）**：完整字段与默认值见 [08](08-config-and-secrets.md) 的配置形状，本篇只点设计要点：
...
frontmatter schema 沿用 [07](07-rules.md)）。
...
- **语言三分**（三者解耦，别把「文件写成什么语言」与「输出 / 记忆用什么语言」绑死）：
  ...
  - **记忆写入语言**：Agent 追加 `MEMORY.md` / `USER.md` 时用用户习惯语言记录，此行为规则须
    显式注入提示词（见 §2 第 8 项、[11](11-i18n.md)）。
- **规则匹配语义与语言无关**。
- **后续可扩展**：Agent 规划器可接本机 agentic CLI（claude / codex 等，复用
  [04](04-pragent-runtime.md) 的本地 CLI provider 思路）作为编排大脑；
