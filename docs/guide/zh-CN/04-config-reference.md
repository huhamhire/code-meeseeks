...
[English](../04-config-reference.md) · **简体中文**
...
| `code_suggestion_spec` | string | `''` | 注入 `/improve`、`/review`、`/ask` 的自由文本规范，用于约束每条建议的写法（软约束）。留空 = 不注入。见下。 |
| `code_suggestion_layout` | string | `''` | 由建议生成评审草稿评论时的确定性 Markdown 排版。留空 = 使用默认排版。见下。 |

#### 代码建议规范与排版

两个可选字段用于控制 AI 代码建议的写法，以及它如何落入评审草稿评论。两者都可在 **设置 → Agent → 策略** 里通过内联编辑器修改，无需手改配置文件。

- **`code_suggestion_spec`** —— 附加到模型指令中的**软**规范，作用于 `/improve`、`/review`、`/ask`。用于引导每条建议的内容结构，例如 _「每条建议按三段组织：问题、分析、建议，各占一行。」_ 模型通常会遵守，但不保证。
- **`code_suggestion_layout`** —— 建议转为评审草稿评论时套用的**确定性** Markdown 模板（精确替换，不依赖模型）。占位符：
...
模板含 `<SUGGESTIONS>` 时作为完整评论正文，否则整段作为前缀拼在正文之前。留空则回退到默认排版：
