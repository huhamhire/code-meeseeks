...
> 最后更新：2026-06-07
> 状态：M0–M4 已交付；M5（打磨 + 多平台扩展，持续中）已落地一批基建项——
> 嵌入式运行时打包 + Windows/macOS 发布、出站 HTTP 代理（模块文档）、首启配置向导、
> get_line_link anchor 根因修复、真实 token 用量采集、多 LLM Provider 适配与验证、
> 设置页连接/LLM/代理可视化 CRUD、单例锁。**下一步主线：GitHub Adapter（多平台扩展）**
>
...
各模块的设计与实现见 [模块设计文档](modules/README.md)。
...
| 维度              | 选择                                                     | 备注                                                                                                   |
| ----------------- | -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| 包管理 / Monorepo | npm (workspaces) + Nx                                    | 见 [模块文档](modules/README.md)                                              |
...
| pr-agent 集成     | 本地 CLI 子进程优先；Docker fallback                     | 见 [模块文档](modules/README.md)                                                      |
| Git 平台（M1）    | Bitbucket Server / DC，REST API v1                       | 见 [模块文档](modules/README.md)                                                  |
| 状态存储          | JSON 文件（原子写）+ `StateStore` 抽象                   | 一期规模小；未来可切 SQLite；见 [模块文档](modules/README.md)           |
| 凭据存储          | 合并在 `config.yaml`，权限收紧；模块级 `SecretStore`     | 用户自负；未来可切 keytar                                                                              |
| 工作目录          | 应用数据固定在 `~/.code-meeseeks/`；**仅 `repos_dir` 可配置** | `repos_dir` 默认 `~/.code-meeseeks/repos/`；见 [模块文档](modules/README.md) |
...
├── state/               # JSON 状态文件（per-PR 目录，详见 模块文档）
│   ├── connections.json
│   ├── watched-repos.json
...
**规则目录** `rules.dir`（可选；用于个性化 PR review 规约，详见 [模块文档](modules/README.md)）：
...
状态文件的 schema、容错与安全设计见模块文档 [03 · 状态存储与数据模型](modules/03-state-storage.md) 与 [05 · 评审→发布闭环](modules/05-review-workflow.md)。
...
- ✅ `@meebox/pr-agent-bridge`：`PrAgentBridge` 策略模式 (`LocalCliBridge` / `DockerBridge`)，统一 `run()` 接口；版本探测 + 自动 fallback；`AbortSignal` 贯通到 spawn 层支持取消（见 [模块文档](modules/README.md)）
...
- ✅ `rules.dir` 规则注入闭合（见 [模块文档](modules/README.md)）：`loadRules` + `pickMatchingRule` 走 frontmatter project/repo/target_branch 正则匹配，命中后填 `PR_REVIEWER__EXTRA_INSTRUCTIONS` 等 env
...
- ✅ 状态持久化：`state/prs/<localId>/runs/<runId>.json` 按 模块文档 per-PR 目录布局；时间戳游标分页 `pragent:listRuns(limit, beforeId)` 支持向上滚动懒加载历史
...
- **状态存储重构 (模块文档)**：原计划 `state/runs/<sanitized-localId>/<runId>.json` + 单文件 `pull-requests.json`；M3 中重构为 hash localId (`sha1(platform|conn|group|repo|remoteId).slice(0,12)`) + per-PR 目录 `state/prs/<localId>/{meta,comments,runs}`。理由：Bitbucket Server 不同 repo 同 PR id 撞 + 评论缓存自然落位 + 路径友好。多平台 identity 字段（`platform / group / repo`）预留 M5
...
- [03 · 状态存储与数据模型](modules/03-state-storage.md)
...
**实际落地**（详见 [模块文档](modules/README.md)）：
...
- **发布成功 = 删本地草稿**：原 模块文档 草稿状态机含 `posted` 终态保留
...
- **草稿外部编辑器打开**：模块文档 标注 M4 之后；长 body 在小
...
- [05 · 评审→发布闭环](modules/05-review-workflow.md)
...
M5 是开放的持续阶段，不设单一 Done when。下面分「已交付」与「进行中 / 待办」两栏滚动维护。

#### 已交付 ✅（截至 2026-06-07）

- ✅ **pr-agent 嵌入式运行时打包**（见 [模块文档](modules/README.md)）：内嵌可重定位 Python 解释器 + 构建期隔离安装 pinned pr-agent；`pr_agent.strategy` 支持 `auto`（默认）/ `embedded` / `local-cli` / `docker`，免除用户装 Python/Docker。`assemble-pragent-runtime.mjs` 组装 + `sitecustomize.py` shim 永久就位（version 守卫兜上游漂移）。`EmbeddedRuntimeBridge` 落地
- ✅ **首发桌面安装包**：electron-builder 出 **Windows x64（NSIS）+ macOS arm64（dmg）**；mac 走 ad-hoc 签名、未公证，见 [mac-build.md](./mac-build.md)。（CI 三平台自动出包矩阵仍待办，见下）
- ✅ **出站 HTTP 代理（[模块文档](modules/README.md)）**：LLM 调用 / 代码平台 REST / git HTTPS 统一走代理，设置页独立模态（地址/端口/Basic Auth + 测试连通），本地地址自动直连。SSH 方式 git 拉取不在此列（走 `~/.ssh/config` 的 `ProxyCommand`）
- ✅ **`/review` finding anchor 根因修复（get_line_link 注入）**：`sitecustomize.py` 猴补 `LocalGitProvider.get_line_link` 返回 `meebox:///<file>#L<s>-L<e>`，让 pr-agent `convert_to_markdown_v2` 走 `[**header**](link)` 分支不再抹掉 file/line；[parse-output.ts](../packages/poller/src/parse-output.ts) 取结构化 anchor，旧 `[file:..., lines:...]` marker / 正文 regex 作行号兜底（`resolveIssueAnchor` 合并两路信号）。anchor 与真实 provider 同源，不再依赖模型自报 marker
- ✅ **多 LLM Provider 适配 + 实测验证**：openai / anthropic / deepseek / dashscope（通义千问）/ volcengine-ark（豆包）已实测验证；ollama / openai-compatible 理论可行待验证。`buildPragentEnv` 按 provider 自动补 litellm 前缀（含 anthropic 原厂模型只填型号名即用）；OpenAI 兼容协议走 `OPENAI__KEY` / `OPENAI__API_BASE` 注入路径。Anthropic 新模型 `temperature` 弃用、marker 破坏 YAML 等上游缺陷由 shim 兜底
- ✅ **真实 token 用量采集**（取代 M3 的 prompt 预估）：shim 采集 litellm usage，ChatPane 输入 / 输出 token 分列展示
- ✅ **首启配置向导**：代码平台连接 +（可选）LLM 引导式配置，降低首次上手门槛
- ✅ **设置页连接 / LLM / 代理可视化 CRUD**（**推翻 M1「放弃 UI 内连接 CRUD」决策**）：首启向导 + 设置页都需要在 UI 内增删改连接 / LLM profile / 代理，故重新实现；草稿态「写入不启用」，底栏保存或显式启用才应用到运行时。「编辑 config.yaml」入口保留给高级用户
- ✅ **单例锁**：二次启动聚焦已有窗口，不再多开
- ✅ **开源发布准备**：README + 开发指南 + Apache-2.0 + NOTICE 第三方声明归集
- ✅ **(M1 残项)** 设置页「编辑 config.yaml」：走 shell 关联程序打开（macOS 实测 VSCode 可用）；不再额外实现 OS 默认文本编辑器分支

#### 进行中 / 待办

- ⏭️ **GitHub Adapter（下一步主线）**：先抽 `PlatformAdapter` conformance 测试套件锁定契约，再实现 GitHub（公有云 + Enterprise Server）。GitLab / Gitea 按需排在其后。数据层 `PrIdentity` 多平台字段（`platform / group / repo`）已预留
- ⏭️ **高阶 Agent 能力**：复杂任务分步规划 + 长期 Memory，从单轮问答走向可累积上下文、多步工具调用
- ⏭️ **AutoPilot 预评审**：轮询发现新 PR 后按规则自动预跑一遍，进应用即见待确认草稿（决策权仍在 reviewer，发布前仍需确认）
- ⏭️ **国际化（i18n）**：多语言界面，优先简体中文 / English
- ⏭️ **规则市场**：导入 / 导出 rules.dir 片段
- ⏭️ **可观测性扩展**：规则命中率、模型对比（token 用量已做）
- ⏭️ **pr-agent run 启动开销优化**：worktree 缓存 / PR 上下文缓存 / 镜像预热 / 容器长驻（详见 [pragent-run-overhead](pragent-run-overhead.md)）
- ⏭️ **大 PR 性能验证**：等真实大样本，实测 Monaco diff + pr-agent 跑通
- ⏭️ **ollama / openai-compatible 验证**：本地模型 / 自部署链路实测
- ⏭️ **凭据存储升级到 keytar**（首次引入 `SecretStore` 抽象 + 替换实现）
- ⏭️ **状态存储按需升级到 SQLite**（替换 `StateStore` 实现，触发条件见 模块文档）
- ⏭️ **(M0 残项)** electron-builder CI 矩阵（Windows / macOS / Linux 三平台自动出包；当前为手工出 Win + mac）
- ⏭️ **离线模式**
...
| JSON 状态文件随使用量膨胀            | M3+    | 监控单文件大小（> 5 MB 告警）；触发条件达成时切 SQLite（见 模块文档）                       |
...
1. ✅ ROADMAP.md + 模块文档 + 模块文档 + 模块文档 + 模块文档
...
7. ✅ M3 pr-agent 集成（A bridge + docker local provider + worktree 物化 → B `/describe` + `/review` + `/ask` + 输出解析 + rules.dir 注入 → C ChatPane 对话式 + 队列模型 + 中断/重试 → D 标签页扩展 + 评论 inline Monaco + StatusBar 队列 popover；状态存储重构走 模块文档）
8. ✅ M4 评审 → 发布闭环（A 草稿池 + DraftZone 行内编辑 → B publishInlineComment + drafts:publishBatch + PR header 入口 → C DraftsPanel 草稿 tab + 文件树 chip + 单条发布 → D 评论 reply/edit/delete + canDelete/canEdit 预判 → E DiffView 跨文件搜索 + LLM 适配 + run meta 模型名 + mergeStatus + 合并按钮；草稿工作流走 模块文档）
9. 🔄 M5 打磨 + 多平台扩展（持续）
   - ✅ 已落地：嵌入式运行时打包（模块文档）+ Win/mac 发布 · 出站 HTTP 代理（模块文档）· get_line_link anchor 根因修复 · 真实 token 用量 · 多 LLM Provider 适配与验证 · 首启向导 · 连接/LLM/代理 UI CRUD · 单例锁 · 开源准备
   - ⏭️ 下一步主线：**GitHub Adapter**（先抽 PlatformAdapter conformance 测试套件锁契约 → GitHub 实现 → GitLab/Gitea 按需）
   - ⏭️ 其余持续项：高阶 Agent / AutoPilot 预评审 / i18n / 启动开销优化 / keytar / CI 三平台矩阵（见 M5「进行中 / 待办」）
