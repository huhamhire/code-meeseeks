...
- ✅ **(M1 残项)** 设置页"编辑 config.yaml"：现走 shell 关联程序打开，已在 macOS 验证可直接用 VSCode 打开，体验达标，**不再额外实现** OS 默认文本编辑器分支（Windows/Linux 如遇关联到非文本编辑器再按需补 `code`/`open -t`/`$EDITOR` 兜底）。**不实现 UI 内连接 CRUD**——直接打开 config.yaml 更适合开发者 + 减少冗余 UI 维护成本
