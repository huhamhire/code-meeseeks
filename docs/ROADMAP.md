...
> 嵌入式运行时打包 + Windows/macOS 发布、出站 HTTP 代理（ADR-0009）、首启配置向导、
...
- [ADR-0009](./adr/0009-network-proxy-support.md) · 出站网络代理支持（HTTP 代理统一作用于 LLM / 代码平台 / git HTTPS）
...
- ✅ **pr-agent 嵌入式运行时打包**（见 [ADR-0008](./adr/0008-pragent-packaging-and-runtime.md)）：内嵌可重定位 Python 解释器 + 构建期隔离安装 pinned pr-agent；`pr_agent.strategy` 支持 `auto`（默认）/ `embedded` / `local-cli` / `docker`，免除用户装 Python/Docker。`assemble-pragent-runtime.mjs` 组装 + `sitecustomize.py` shim 永久就位（version 守卫兜上游漂移）。`EmbeddedRuntimeBridge` 落地
...
- ✅ **出站 HTTP 代理（[ADR-0009](./adr/0009-network-proxy-support.md)）**：LLM 调用 / 代码平台 REST / git HTTPS 统一走代理，设置页独立模态（地址/端口/Basic Auth + 测试连通），本地地址自动直连。SSH 方式 git 拉取不在此列（走 `~/.ssh/config` 的 `ProxyCommand`）
...
- ⏭️ **状态存储按需升级到 SQLite**（替换 `StateStore` 实现，触发条件见 ADR-0003）
...
9. 🔄 M5 打磨 + 多平台扩展（持续）
   - ✅ 已落地：嵌入式运行时打包（ADR-0008）+ Win/mac 发布 · 出站 HTTP 代理（ADR-0009）· get_line_link anchor 根因修复 · 真实 token 用量 · 多 LLM Provider 适配与验证 · 首启向导 · 连接/LLM/代理 UI CRUD · 单例锁 · 开源准备
