...
- ✅ 草稿池持久化：`state/prs/<localId>/drafts.json` per-PR；状态机
  ...
  deleteDraft / listDrafts` 在 `@pr-pilot/poller`；reactive store
...
- **`/improve` 永久屏蔽**：实测 pr-agent `/improve` 工具依赖在线平台
  ...
  practices 集成，跟 pr-pilot 本地 PR 管理路径不兼容。ChatPane COMMANDS
