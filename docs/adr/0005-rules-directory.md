...
- **实现包**：[`@pr-pilot/rules`](../../packages/rules/)

## 背景

pr-agent 默认 prompt 是通用的，缺乏对**具体团队 / 仓库**编码规约的感知（命名约定、必须有单元测试、特定库用法、安全清单等）。pr-agent 支持 `--extra_instructions`（per-tool 配置 / env，例如 `PR_REVIEWER__EXTRA_INSTRUCTIONS`）把自定义提示拼到 system prompt 上，但需要 pr-pilot 来：
...
- 规则要能**用 git 版本控制**、跟团队共享 —— 不能挤在 `~/.pr-pilot/` 个人配置里
...
- **规则 lint / 预览**：CLI 工具 `pr-pilot rules check` 离线 dry-run 一个 PR 看哪条规则命中
