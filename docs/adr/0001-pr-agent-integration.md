...
meebox 是 Electron + Node + TypeScript 应用。两者技术栈不同，必须确定一个稳定的集成方式。

约束：

- 用户机器**不一定预装 Python**，但**大多数会装 Docker** 或愿意装 Python。
- 升级 pr-agent 应当**对 meebox 主体代码零影响**。
...
`pr-agent` 装在用户的 Python 环境中，meebox 通过 `child_process.spawn('pr-agent', [...])` 调用。
...
把 pr-agent 包成本地 HTTP 服务（FastAPI 简单封一层），meebox 通过 HTTP 调用。
...
把 pr-agent 的 prompt + 编排逻辑用 TS 重写，meebox 直接调 LLM API。
...
- pr-agent 升级 = 改一个版本号，不需要改 meebox 业务代码
