...
/**
 * 自由规划（ReAct）的「单步」步骤（见 docs/arch/06-agent.md「会话 Agent 化」）：每轮 chat 规划下一动作
 * （调工具 / 收尾），解析 JSON 动作、红线硬校验后分发工具并把结果回喂；中途输入并入 progress、计划随轮维护
 * 与重排。规划是单步循环 —— planner.ts 的驱动反复跑本步直至 final / 步数上限。提示词组装与解析就近放在此。
 */
 ...
 * （视中英文占比），对应约 32k~64k token 半窗的目标量级。
 */
 ...
 * 思考前后各检一次（思考阶段也能即时停）。驱动重复调用本步直至 final / 步数上限。
 */
export const planCycleStep: StepHandler<PlanStepCtx, PlanCycleOutcome> = {
  name: 'plan-cycle',
  run: async (ctx) => {
