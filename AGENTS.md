...
- **嵌入式 pr-agent 运行时**（见 [docs/arch/02-agent/03-pragent-runtime](docs/arch/02-agent/03-pragent-runtime.md)）：`apps/desktop/scripts/assemble-pragent-runtime.mjs` 按 `pragent-runtime.json` 把可重定位 CPython + pinned pr-agent 装到 `apps/desktop/vendor/pragent/`（gitignored）。
